...
| Chrome tự tắt tiếng sau ~15 giây | Bug đã biết của Chrome/Edge. `tts-reader.js` cắt utterance theo `maxLen() = 170 × rate` (clamp 160–320 ký tự) nên MỌI utterance đều ~10 s. **KHÔNG dùng nhịp `pause()`/`resume()`** — cách đó khử được bug nhưng làm giọng GIẬT rõ rệt |
| Giọng đọc nghe VẤP / ngắt quãng | Nguyên nhân là mỗi lần `speak()` một utterance mới là một khoảng lặng. `tts-reader.js` khắc phục bằng **hàng đợi nạp trước**: `fillQueue()` luôn giữ ~600 ký tự đã `speak()` vào engine (nạp trước NHIỀU đoạn, không phải một). Test: `[A]` phải nạp sẵn > 1 utterance, `[C]` hàng đợi KHÔNG được rỗng giữa chừng |
| Giọng nghe máy móc dù đã hết vấp | Chất lượng do GIỌNG quyết định, không phải code. Bộ sắp xếp ưu tiên giọng **mạng** (`localService === false`, thường là bản neural) trước giọng compact cài sẵn |
...
| Ảnh trong repo **đặt SAI tên / SAI chỗ**, hash MD5 KHÔNG bắt được | Ảnh repo có thể được render lại bằng `pdftoppm` (khác `pdfimages`) nên **MD5 luôn lệch**. Cách bắt: đối chiếu **KÍCH THƯỚC PIXEL** rồi **xem trực tiếp bằng tool Read**. Đã tìm ra `asset-polygon-count.png` thực chất là bản thấp-res của `asset-mesh-import-settings.png`, nhúng dưới mục "polygon count" suốt nhiều lần audit |
| Nguồn WEB: Jina reader **KHÔNG trả về phần COMMENT** (WordPress/Discourse) | Nhiều đính chính quan trọng nằm trong comment — kể cả đính chính của CHÍNH tác giả. Phải `curl` HTML thô rồi bóc từ `id="comments"`. Ở M2 có **4/12 lỗ hổng** nằm trong 43 comment của jacksondunstan |
| Lọc ảnh PDF theo ngưỡng **40 KB** làm MẤT ảnh | Ảnh **index-PNG** nhỏ hơn nhiều so với JPEG cùng nội dung (có ảnh chỉ 32 KB nhưng là bảng setting đầy đủ). Chạy `pdfimages -list` xem loại ảnh TRƯỚC rồi mới lọc |
| Ảnh bị **HOÁN VAI** giữa hai caption liền nhau | Xảy ra khi chương của e-book có ảnh rải trên nhiều trang lệch với thứ tự caption. Ở M2 hai ảnh Animation bị đổi chỗ cho nhau. Chỉ phát hiện được bằng cách **xem tận mắt từng ảnh** rồi đối chiếu với caption — không script nào bắt được |
| Nguồn WEB: Jina reader **bỏ hết `<img>`** và **không lấy được reply của Discourse** | Ảnh: tải HTML thô bằng `curl` rồi grep `<img`. Reply Discourse: dùng JSON API `https://discussions.unity.com/t/<id>.json` với `-H "Accept: application/json"` (thêm `?print=true` bị chặn *"Crawler is not allowed!"*) |
| `educba.com` chặn Jina (403 / Cloudflare) | Dùng `curl` TRỰC TIẾP với `-A "Mozilla/5.0 …Chrome/120…"` **và** `-H "Accept-Language: en-US,en;q=0.9"`, rồi convert bằng `html-to-text` |
| Audit ảnh báo "còn X ảnh chưa dùng" nhưng thực ra KHÔNG thiếu | Phải **GIỚI HẠN theo PHẠM VI TRANG** mà Module thực sự dùng. Ví dụ Field Guide 79 trang nhưng M5 chỉ dùng tr.18–28; Mobile Perf chỉ tr.22–23 + 48–49. Không scope thì ảnh của Module KHÁC bị đếm là "thiếu" |
| Đếm `Figure N.M` / `Table N.M` để đo độ phủ ⇒ **RẤT NHIỀU dương tính giả** | Tài liệu viết bằng tiếng Việt nên không giữ nhãn `Table 3.1`; nội dung VẪN có. Phải grep **nội dung** (tên cột, con số) chứ không grep nhãn |
| Lo `pdfimages` bỏ sót **hình VECTOR** | Kiểm bằng script: liệt kê trang có caption `▲ Figure` nhưng KHÔNG trích được ảnh raster nào. Với 4 e-book của M5 kết quả là **0 trang** ⇒ không có điểm mù. Nếu có, dùng `pdftoppm` render cả trang rồi crop |
